...
namespace ForgeAnalytic.Generated.entropy_positive_chart
private noncomputable def q0 (x : ℝ) : ℝ := (((1 : ℝ) / 1) * x ^ 0 * Real.exp (((0 : ℝ) / 1) * x)) + (((-1 : ℝ) / 1) * x ^ 0 * Real.exp (((1 : ℝ) / 1) * x)) + (((1 : ℝ) / 1) * x ^ 1 * Real.exp (((1 : ℝ) / 1) * x))
private noncomputable def q1 (x : ℝ) : ℝ := (((1 : ℝ) / 1) * x ^ 1 * Real.exp (((1 : ℝ) / 1) * x))
private noncomputable def q2 (x : ℝ) : ℝ := (((1 : ℝ) / 1) * x ^ 0 * Real.exp (((1 : ℝ) / 1) * x))
private noncomputable def q3 (x : ℝ) : ℝ := (0 : ℝ)

private theorem h3 (x : ℝ) (_hx : 0 ≤ x) : 0 ≤ q3 x := by
  simp [q3]
...
private theorem h1 (x : ℝ) (hx : 0 ≤ x) : 0 ≤ q1 x := by
  apply ladder_step (f := q1) (g := q2) (a := 0) (rho := ((1 : ℝ) / 1))
  · intro x
...
theorem nonnegative (x : ℝ) (hx : 0 ≤ x) :
    0 ≤ (((1 : ℝ) / 1) * x ^ 0 * Real.exp (((0 : ℝ) / 1) * x)) + (((-1 : ℝ) / 1) * x ^ 0 * Real.exp (((1 : ℝ) / 1) * x)) + (((1 : ℝ) / 1) * x ^ 1 * Real.exp (((1 : ℝ) / 1) * x)) := h0 x hx

#print axioms nonnegative
end ForgeAnalytic.Generated.entropy_positive_chart
