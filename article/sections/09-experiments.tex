...
\subsection{Nine runs, reported separately}

This section reports nine independent prototype runs, labelled \src{p1}
to \src{p9} after the source proposals listed in
Appendix~\ref{app:provenance}. They are reported separately and they are
\emph{not} pooled, for a reason stronger than caution.

All nine ran on the same day in the same kind of environment --- Python 3.13.5
...
260 pytest tests passed, the highest count of the nine. No seed recorded.
...
This run has the most systematic mutation testing of the nine, with parametrised
...
\subsubsection*{What the nine runs recorded}

None of the nine compiled any Lean source. Each recorded the same reason --- no
\code{lean} or \code{lake} executable in the authoring environment --- under five
different filenames and in two different formats. Consequently none of them
performed a kernel check, an axiom audit, or any comparison against \grind{},
Aesop, \code{nlinarith}, LeanHammer, or any other tactic. Twenty-one
\code{.lean} files across the nine, totalling about 3{,}000 lines, were
\emph{candidate source}.
...
Three of the sixteen files in the merged tree import nothing beyond Lean core
and can therefore be elaborated without a built Mathlib:
...
about the other thirteen files, which import Mathlib and need a built project.
It does not establish that any certificate checker is correct. And it is not a
comparison against any Lean tactic --- successful elaboration of a hand-written
example proves that the example type-checks, not that automation found it.
...
cannot; and that three Lean design artefacts type-check under the pinned
toolchain.
