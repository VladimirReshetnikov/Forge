...
\subsection{Twenty-seven runs, reported separately}

This section reports twenty-seven independent prototype runs in three rounds:
the design round \src{p1}--\src{p9}, covered first; the extension round
\src{e1}--\src{e9}, covered in Section~\ref{sec:extension-runs}; and the
third round \src{r1}--\src{r9}, covered in
Section~\ref{sec:round-three-runs}. All twenty-seven are named after the
source proposals listed in Appendix~\ref{app:provenance}. They are reported
separately and they are \emph{not} pooled --- neither within a round nor
across the three --- for a reason stronger than caution.
...
\subsubsection*{What the twenty-seven runs recorded}

None of the twenty-seven compiled any Lean source. Every one of them recorded
the same reason --- no \code{lean} or \code{lake} executable in the authoring
...
3{,}000 lines, twenty across the extension round, totalling about 3{,}100, and
sixteen across the third round, totalling about 750, were \emph{candidate
source}. That three independent rounds produced this much uncompiled Lean is
itself worth noticing: the bottleneck was never the amount of Lean anyone was
willing to write.

One of the third round declines to participate. \src{r9} ships no Lean at all,
and says why: it ``deliberately contains no placeholder theorem files presented
as implemented proofs''. That is a pointed remark about the practice the other
twenty-six follow, and the next subsection shows it landing.
...
proposal ships none. That leaves three, and for the first time in twenty-seven
proposals a compiler had something to say about delivered Lean.
...
nine third-round suites do the same and that their stored objects replay; and
that eighteen Lean artefacts --- three from the design round, ten from the
extension round, three the merge wrote by deduplicating those ten, and two from
the third round --- type-check under the pinned toolchain, eleven of them
printing \emph{does not depend on any axioms} for every theorem they expose.
...
claims across the twenty-seven prototypes should ask which of these three things
the sentence means.
