...
Re-auditing all ninety Lean files across the four rounds and the merged
tree with the corrected scanner: \emph{none} contains a real \code{sorry} or
\code{sorryAx}. All twenty-six occurrences of the token are authors stating
...
\subsection{A checker, and the first tactic comparison}

Every Lean file discussed above is a \emph{specimen}: a statement of what a
checker would have to prove. \code{Forge.Checker} is a checker. It is one
certificate family of thirteen, and reporting what it cost is more useful than
reporting that it exists.

\subsubsection*{What was built}

A checker for cone certificates --- sums of squares, products of the inequality
constraints, and equality multipliers --- with \code{Cert.check} a total
Boolean function performing no search, and \code{Cert.sound} proving that
acceptance implies nonnegativity at every assignment satisfying the
constraints. Underneath, evaluation is proved to be a ring homomorphism and the
zero test is proved to decide identical vanishing; the latter is what licenses
quantifying over all assignments, because a certificate asserts an
\emph{identity} and identity is stronger than agreement anywhere finite.

The prototype's three cone certificates are exported by
\code{tools/export\_lean\_cone.py}, cleared of denominators, re-checked in
Python, and then accepted by the Lean kernel through \code{decide} rather than
\code{native\_decide}. The axiom footprint across all three files is
\code{propext} and \code{Quot.sound}, with no \code{Classical.choice} and no
\code{sorryAx}. \code{ForgeCore} now builds as a library.

Three constraints shaped it. It is core Lean only, because fifty-eight of this
repository's ninety Lean files then depended on Mathlib and none had ever been
checked --- a checker in that bucket would have been one more uncompiled claim.
Coefficients are integers, because \code{Int} is in core and \code{Rat}'s
lemmas are not; clearing denominators is a change of representation, and every
multiplier introduced is positive, so the hypotheses transfer exactly. And each
certificate needs a second, generated theorem in ordinary arithmetic, because
$\code{0} \le \code{eval x target}$ for a \code{Poly} literal is not a goal anyone would
type.

\subsubsection*{The comparison, and why it is weaker than it first looks}

Section~\ref{sec:evaluation} asks for a comparison against existing tactics.
None of the thirty-six proposals ran one. Here is the first, on the three goals
these certificates settle. Mathlib results are from a v4.32.0 build, so the
toolchain differs from the v4.34.0 used everywhere else in this document.

\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}llll@{}}
\toprule
Tactic & \src{hidden\_quadratic} & \src{equality\_constrained} & \src{guard\_product} \\
\midrule
\code{grind} (core)      & fails & fails & fails \\
\code{omega} (core)      & fails & --- & --- \\
\code{positivity}        & fails & fails & \textbf{passes} \\
\code{nlinarith}, bare   & fails & fails & \textbf{passes} \\
\code{nlinarith} + the certificate's squares & \textbf{passes} & \textbf{passes} & passes \\
\code{linarith} + the certificate's squares  & \textbf{passes} & fails & --- \\
\bottomrule
\end{tabular}
\caption{Mathlib v4.32.0; core tactics at v4.34.0. Every failure was a prompt
refutation-search failure, not a timeout.}
\label{tab:tactic-comparison}
\end{table}

The sharpest row is the last. For \src{hidden\_quadratic}, once the squares are
supplied, plain \code{linarith} closes the goal --- the certificate is
\emph{linear} in its squares, so no nonlinear search is needed at all. That is
as clean a separation as this experiment can give between the certificate being
the content and the search being the hard part.

Four qualifications, and they matter more than the table.

\emph{\code{grind} is a weak baseline.} It fails all three, including
$0 \le a * b$ from $0 \le a$ and $0 \le b$ --- its cutsat model
returns the assignment $a=b=0$ and it never applies multiplication
monotonicity. Beating it is not evidence of much, and this document should not
lean on that row.

\emph{One of the three goals is not evidence at all.} \src{guard\_product} is
$0 \le a*b$ from two nonnegativity hypotheses. Bare \code{nlinarith} and
\code{positivity} both get it unaided, and its Forge certificate is the trivial
one. It belongs in the table only so that the table is not filtered.

\emph{The gap is smaller than ``with the certificate versus without''.} Given
only \emph{two} of \src{hidden\_quadratic}'s three squares, \code{nlinarith}
still succeeds: it manufactures the missing $b^2$ term through its own product
heuristics. So what was measured is ``some of the certificate's squares'' against
``none of them'', not ``exactly this certificate'' against ``none''.

\emph{Two goals out of three is a small sample.} Nothing there is a benchmark.

\subsubsection*{Thirty-six more, and what the zero actually means}

So a benchmark was built: thirty positive problems with certificates
constructed rather than searched for --- one to three variables, degrees two
through six, two to four squares, targets of three to twenty-six terms, and
coefficients spanning eight orders of magnitude --- together with six negative
controls. The controls matter more than the positives: a benchmark on which
everything passes has measured nothing. Between them they exercise all five
conjuncts of \code{Cert.check}, and two of them satisfy the polynomial identity
\emph{exactly} while negating the target and a weight, so each would certify
$0 \le -p$ for a positive-definite $p$ if its guard were removed.
...
Measurement & \\
\midrule
Positive problems verified by the checker & 30/30 \\
Negative controls correctly rejected & 6/6 \\
Proved unaided by core \code{grind} & \textbf{0/30} \\
Proved unaided by core \code{omega} & \textbf{0/30} \\
Whole-file elaboration & 11.7\,s \\
Axioms, across all 96 theorems & \code{propext}, \code{Quot.sound} \\
\bottomrule
\end{tabular}
\caption{Lean v4.34.0, core only. No timeouts: every tactic failure was
reported in one to three seconds.}
\label{tab:bench}
\end{table}

\textbf{The zero is not what it looks like, and the calibration probes are the
real result.} Before reading 0/30 as a gap, ask what \code{grind} can do on the
easiest possible member of the family:
...
$0 \le x \cdot x$ & fails \\
$0 \le 3x^2 + 1$ & fails \\
$0 \le xy$ given $0 \le x$, $0 \le y$ & fails \\
$0 \le 2x + 5 - 2x$ & \emph{proves} \\
\bottomrule
\end{tabular}
\end{center}

\code{grind} cannot prove that a square is nonnegative. Its own diagnostics say
why: cutsat treats $x^2$ as an unconstrained atom and assigns it zero, and no
e-matching pattern supplies the missing fact. No problem in this family was ever
reachable, so the 0/30 is a statement about \code{grind}'s scope and not about
the difficulty of the problems. The same applies to \code{omega}, which is
documented as linear. Neither is a defect in those tactics, and neither result
should be quoted as Forge outperforming them. The tactics that \emph{do} address
this territory --- \code{positivity}, \code{nlinarith}, \code{polyrith} --- live
in Mathlib, which this development deliberately does not import.

\textbf{Kernel cost is driven by polynomial size, not coefficient size.} Three
matched pairs differing only in coefficient magnitude, by factors up to
$2.4\times10^{8}$, show net elaboration times of $0.202 \to 0.139$\,s,
$0.137 \to 0.136$\,s and $0.079 \to 0.070$\,s: no cost at all, because
\code{Int} bottoms out in GMP-backed literals and the kernel never sees a large
integer as large. What does drive cost is the number of terms, and
superlinearly: three terms in 0.02--0.04\,s, six in 0.07--0.14\,s, thirteen in
0.18--0.28\,s, twenty in 0.46\,s, twenty-six in 0.73\,s. Roughly nine times the
terms for twenty-four times the time, which is what the quadratic
\code{insertTerm} in \code{collect} predicts. The scaling limit of this checker
is polynomial size --- variables $\times$ degree $\times$ squares --- and a
better \code{collect} is the obvious first optimisation if one is ever needed.

\begin{remark}[The benchmark found a bug in the exporter]
Three-variable problems exposed a defect that the two-variable corpus could not.
\code{lean\_concrete} emitted a monomial's factors left-associated,
\code{(x0 * x1 * x2)}, while \code{monoEvalFrom} recurses on the tail and builds
\code{x0 * (x1 * x2)}. \code{omega} atomises both, sees two unrelated terms, and
rejects a true goal. A monomial with at most two factors associates the same way
in either direction, so every certificate in \code{Corpus.lean} sits exactly on
the blind spot.

It is fixed at the source and the benchmark's local workaround removed. The
general point is the one worth keeping: the corpus that was available could not
have found this, and the defect was in the \emph{bridge} rather than in the
checker or its soundness proof --- the part that translates a verified statement
into the goal someone actually wants, which is the part with no theorem
attached to it.
\end{remark}

\emph{What the benchmark does not establish.} The certificates were constructed,
not found, so nothing here measures search cost --- and search is the expensive
half. Random sums of squares are generically strictly positive, so no hard
instance sits near the boundary of the cone and nothing Motzkin-like is present.
The timings are single runs on one machine and two runs of identical work
differed by up to 30\%, so the ratios carry the argument and the absolute
figures do not.


\begin{remark}[The comparison found an error, and it was in the prompt]
Setting this up, the equality-constrained certificate was transcribed with its
multiplier's constant term negated --- the equality polynomial copied into the
multiplier slot. The identity then fails by $2a+2b-2$. Checking against the
stored artifacts showed both of them correct: the prototype's bundle records the
multiplier as $\tfrac12(1+b+a)$ and \code{Corpus.lean} records its cleared
integer form, and
$\tfrac12(b-a)^2 + \tfrac12(1+b+a)(-1+b+a) = a^2+b^2-\tfrac12$ exactly.

The error was in the transcription, and it survived until an independent tool
expanded the polynomial. This is the same lesson as the \code{sorry} scanner and
the axiom line: a hand-copied identity is a claim, and the tools in this
repository exist because claims of that kind are wrong at a rate that intuition
underestimates.
\end{remark}

\subsection{Gate 2 in core Lean, and what reviewing it found}

After the cone checker, the rest of Gate~2 was built as four streams --- typed
reification and a tactic, the oracle protocol, the affine and Farkas families,
and the recurrence families --- and then reviewed adversarially before any of it
...
\toprule
Component & Proved or measured \\
\midrule
Typed reification & \code{eval\_toPoly}: evaluation of a reified polynomial is its denotation \\
\code{forge\_cone} & closes $0 \le e$ and $a \le b$ goals from a certificate via \code{cone\_denote} \\
\code{forge\_cone?} & oracle round trip $\approx$420\,ms, kernel check $\approx$150\,ms, isolated \\
Affine witness & \code{AffineCert.sound}, and \code{check\_iff}: the check is exact \\
Farkas & \code{FarkasCert.sound}; three prototype refutations, generated \\
Polynomial recurrence & \code{RecCert.sound} for every $n$; five power sums \\
Conserved invariant & \code{InvCert.sound} at every reachable state \\
Axiom audit & 673 theorems within \code{propext}, \code{Quot.sound}, one exemption \\
\bottomrule
\end{tabular}
\caption{Lean v4.34.0, core only. The oracle figures are isolated calls; inside
a test file they read ten times higher because parallel elaboration starts ten
oracle processes at once.}
\label{tab:gate2}
\end{table}

\subsubsection*{Review}

Three reviewers were each asked to make one part accept something false and to
report only what they had reproduced. The tactic and oracle review ran
forty-five attacks, including a hostile oracle of its own; the affine and Farkas
review ran twenty attacks and a 4000-case differential fuzz against the Python
checker, with no disagreement; the recurrence review ran more than twenty. None
produced a false acceptance. The printed proof terms show certificates entering
only as closed literals that the kernel re-checks.

Every defect found lay outside the kernel-checked part.

\emph{Injection at the data boundary.} Every exporter wrote the bundle's record
id into a Lean comment. An id containing the comment terminator closed it, and a
generated corpus compiled cleanly while proving $1 = 2$ from an injected axiom.
The real theorems never depended on it, but a generated \code{axiom \ldots{} :
False} poisons whatever imports it, and Gate~2 names mutation rejection at the
data boundary as an exit criterion. A shared emission guard now refuses
non-identifier ids and audits every generated file before writing it; all
corpora regenerate byte-identical through it.

\emph{Untrue bounds.} The oracle protocol advertised a thousand squares and a
thousand terms per polynomial. Measurement shows the kernel's limit tracks the
\emph{expanded} identity instead: a thousand trivial squares check in seconds,
while one square of forty-five terms does not. A univariate square passed at an
expanded size of 1679 and failed at 2114, and the limit is now set at 1600,
documented as measured. A valid certificate beyond it is refused before checking
and reported as not checked, where before it was reported as rejected.

\emph{Smaller defects.} Atom matching could assign metavariables in the user's
goal; numerals were read without checking their instance, so the oracle could be
sent a different problem from the goal; the command resolver could replace the
Python interpreter with a stray file of the same name; the conserved-invariant
scale guaranteed nothing its documentation implied; and an exporter claimed a
certificate cannot choose the problem it certifies, which review disproved by
exporting a copy with a different initial point.

\emph{Composition with \code{grind}.} Gate~2 says this stage adds useful
nonlinear facts to a stock \code{grind} leaf. A goal wrapping linear
bookkeeping around a nonlinear core is proved by neither \code{grind} nor
\code{omega} alone, and by both once \code{forge\_cone} supplies the one fact
through a \code{have}. A first draft of that test included a hypothesis from
which the fact followed linearly; \code{grind} proved it unaided, and the
\code{fail\_if\_success} line that exposed this is kept in the test.

\subsection{A head-to-head on problems Forge did not choose}

Everything above shows the checker is sound. It does not show the search is
useful: the earlier comparison ran on the prototype's own certificates, and the
benchmark's certificates were constructed before its problems. So the search was
run against Mathlib's nonlinear tactics on twenty well-known polynomial
inequalities --- AM-GM, Cauchy--Schwarz in two dimensions, $a^4+b^4+c^4 \ge
abc(a+b+c)$, Schur's inequality, constrained forms --- plus two deliberate
probes: Motzkin's polynomial, nonnegative but not a sum of squares, and a
degree-six sum of squares past the oracle's degree bound. Each was checked true
on an integer grid before any Lean ran.
...
Tactic & Solved (of 20) \\
\midrule
\code{forge\_cone?} & \textbf{12} \\
\code{nlinarith}, no hints & 3 \\
\code{positivity} & 1 \\
\code{grind}, \code{omega} & 0 \\
\midrule
Solved by \code{forge\_cone?} only & 8 \\
Solved by a Mathlib tactic only & 0 \\
\bottomrule
\end{tabular}
\caption{Lean v4.32 with Mathlib, the only built Mathlib available; Forge's core
compiles unchanged on it. Sequential elaboration, one heartbeat limit for every
attempt. Data in \texttt{results/tactic-headtohead.json}.}
\label{tab:headtohead}
\end{table}

The failure reasons were read before the table was trusted, because a harness
defect would produce the same counts. Every \code{nlinarith} failure was a
genuine refutation failure in well under a second, not a timeout. It received no
hints on purpose: given the certificate's squares it succeeds, as the earlier
comparison showed, so what is being measured is \emph{finding} a certificate.
Fourteen of the twenty \code{positivity} failures are rejections of goal form, not attempts; on the six goals already of the form $0 \le e$ it solved one. All
sixteen successes are real proofs: the eighty-four failures produced exactly
eighty-four errors and there was no \code{sorry}.

Forge's own failures are as informative as its successes. Six were its search
returning no candidate, including $(x-y)^4$ written out and $x+y=2 \Rightarrow xy
\le 1$ --- textbook sums of squares a semidefinite search would find --- and two
were its degree bound. None was a size refusal: the kernel limit measured earlier
did not bind here. When both succeeded, Forge was slower, because its oracle is a
separate process.

One restriction of that design was lifted afterwards. Soundness was proved at
integer points only, because core Lean has no reals; but a certificate asserts a
polynomial identity, which holds in every commutative ring, so the same Boolean
check proves the real statement. A Mathlib file proves exactly that, reusing
every existing check unchanged, and restates the prototype's three certificates
over the reals --- among them $x + y = 1 \Rightarrow 2x^2 + 2y^2 \ge 1$, which is
tight at $x = y = \tfrac12$. It was the first Mathlib-dependent file in this
repository that anything had checked, compiled with the same toolchain as the
head-to-head; its theorems depend on \code{Classical.choice} as well, because
Mathlib's reals do. Once the Mathlib revision the build file pins had been
fetched, the thirteen delivered Mathlib files merged into the tree were compiled
on the pinned toolchain as well. Twelve passed as delivered; one declared
real-valued functions without marking them noncomputable, which Lean rejects,
and passed once they were. An audit that walks the environment finds the 332
theorems of these fifteen files within \code{propext}, \code{Classical.choice}
and \code{Quot.sound}. The proposals' own copies and the prototype's emitted
files, 44 more, were then compiled the same way: 34 pass. Of the rest, one
places a module docstring above its import, one lacks the same
noncomputable marking, two are proof scripts that no longer survive this
Mathlib, and six import one of those. The prototype's emitter had the
noncomputable defect too, and was fixed.

The comparison was run to decide whether the search deserves a planner built
around it. It does. But the same run shows the search's gaps plainly, and those
are cheaper to close, measurable with the same harness, and inherited by any
planner --- so they came before Gate~1.

\subsubsection*{Closing the gaps, and running it again}

The six search failures had two causes. The dictionary proposed only binomial
squares guessed from the target's even monomials, so $1 - xy$ never suggested
$(x-y)^2$; offering all binomial squares removes that. And no dictionary proposes
a trinomial with specific coefficients, such as $(x^2 - 2xy + y^2)^2$ for
$(x-y)^4$. For that a Gram-matrix search was added: the linear constraints on a
Gram matrix are solved exactly, a positive semidefinite point is found
numerically by alternating projections --- no semidefinite solver is available
--- and a rational matrix is reconstructed exactly from it, decomposed by exact
$LDL^\top$ and checked. The degree bound rose from four to six.

On the same twenty problems, \code{forge\_cone?} then solved eighteen, fourteen of
them solved by no Mathlib tactic tried; Mathlib's counts were identical in both
runs. The two failures left are Motzkin's polynomial, which is not a sum of
squares, and Schur's inequality, which needs reasoning outside this certificate
family. One caveat outweighs the rest: the search was improved in response to the
first run's failures on these same problems, so the second run shows the fixes
work where they were aimed, not how the search generalises. That needs a problem
set chosen after the fixes.

\subsubsection*{A held-out set}

So one was written: twenty new problems chosen to differ from the first set ---
Cauchy--Schwarz in three dimensions with six variables, products of hypotheses,
a power of a single hypothesis, an equality with a nonlinear multiplier,
degree-six forms, and Choi--Lam's polynomial as a second probe that is not a sum
of squares --- committed and pushed before any tool ran on them, with the rule
that the search would not be changed in response.

\code{forge\_cone?} solved nineteen of the twenty; \code{nlinarith} solved three,
\code{positivity}, \code{grind} and \code{omega} none. Sixteen were solved by
Forge and by no Mathlib tactic tried, and none the other way. The single miss was
Choi--Lam's polynomial, for the right reason: no positive semidefinite Gram
matrix exists to reconstruct. The same checks as before held --- every
\code{nlinarith} failure genuine, every success a kernel-checked proof.

This is the strongest evidence in this document that the search is useful. It is
bounded all the same: twenty problems, curated by the hand that wrote the search,
over the integers, in at most six variables. It shows the fixes were not fitted
to the first set. It does not show how the search fares on goals drawn from real
proofs, which is where a planner would put it to work.
