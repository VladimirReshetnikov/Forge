...
\begin{remark}[An available contribution]
That gap is a genuine opportunity rather than a fact of nature. \textsf{LJT} is
contraction-free and terminating, so a failed search tree is \emph{finite} and
could be reified into a checkable refutation object. Nobody appears to have
built that. Doing so would give this design a second
\emph{certificate-producing} infeasibility worker, in a fragment --- arrows,
products, constructor-tagged sums, empty types, over opaque atoms --- where
users regularly believe something is impossible and get no confirmation.
\end{remark}
