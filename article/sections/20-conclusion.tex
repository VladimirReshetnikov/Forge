...
Five complementary kinds of progress are available: discovering a certificate,
discovering an invariant, constructing a witness, computing a \emph{closure}
that makes an unbounded family finite, and --- the third round's addition ---
returning a \emph{quantity} rather than a verdict. Each of them changes what
...
The recommendation below is unchanged by the third round, and that is itself
worth recording: nine more proposals, three new subject areas, and the cheapest
end-to-end target is still the one the extension round identified. The
...
Established: twenty-seven prototype runs synthesise certificates in fragments
...
games, optimal transport and unbounded stacks; every stored certificate replays
without the search libraries; the specified mutation and corruption tests are
rejected; several mechanisms solve problems their own restricted precursors
cannot; all eighteen later suites re-run in a different environment and
reproduce their recorded counts; and eighteen Lean artefacts elaborate under the
pinned v4.34.0 toolchain, eleven of them with no axiom dependencies.
...
prototype experiments can close, and two further rounds are the evidence for
that: eighteen more independent efforts, more certificates, more mutations,
more proved theorems on paper --- and the Lean column still reads
\code{NOT\_RUN} in every one of the twenty-seven source packages. Three rounds
have now produced roughly seven thousand lines of candidate Lean and eighteen
compiled files, all of which were compiled here rather than by any author. The
gap closes only by writing the Lean implementation described in
Section~\ref{sec:integration} and running the evaluation described in
