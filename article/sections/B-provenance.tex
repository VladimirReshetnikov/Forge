...
This document is a merge of eighteen independently prepared proposals for the
same system, in two rounds. The \emph{design round} \src{p1}--\src{p9} was
merged first. The \emph{extension round} \src{e1}--\src{e9} was then prepared
against that merged draft, at commit
\texttt{674521027d968d59f7b83220ed52304a85cb55e2}, and merged in turn. All
eighteen are retained verbatim under \code{proposals/} so that every claim here
can be audited against its source. Almost nothing in this article is new
mathematics: each statement is carried over from at least one of the eighteen,
or from the two external sources named in \S\ref{sec:external-sources}, and
this appendix records which.

The second round is not a second opinion on the first. Its proposals read the
merged draft, took its capabilities as prerequisites rather than contributions,
and several of them say so in their opening pages. Where this appendix records
...
\S10 Experiments & Each run reports itself. The re-run table and the Lean
elaboration tables are the merge's own work, not any proposal's. \\
\S11 Evaluation &
...
Fourth, and the only new evidence: sixteen Lean files were compiled
(Tables~\ref{tab:lean}, \ref{tab:lean-extensions} and~\ref{tab:lean-closure}),
and all nine extension suites were re-run and reproduced their recorded counts
(Table~\ref{tab:extension-rerun}). Every one of the eighteen sources recorded
\code{NOT\_RUN} for Lean. That is now narrower --- six of the twenty-one files
in the merged tree and ten of the twenty extension-round source files elaborate
under Lean v4.34.0, ten of the sixteen with no axiom dependencies at all --- and
the limits of what it shows are stated where the result appears. Three of those
...
Fifth, and specific to the second round: the extension proposals were prepared
...
or proposed for production. \src{pN} and \src{eN} labels attribute a result to
the design-round or extension-round run that produced it. \unknown{} always means a bounded search found nothing; it never
